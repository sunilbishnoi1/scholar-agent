...
class PaperRetriever:
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                if e.code == 503:
                    ...
                    logging.warning(f"arXiv service unavailable (503), waiting {wait_time}s before retry {attempt + 1}/{self.MAX_RETRIES}")
                ...
                logging.warning(f"Network error fetching from arXiv: {e.reason}. Retry {attempt + 1}/{self.MAX_RETRIES} after {wait_time}s")
                time.sleep(wait_time)
    ...
    def _search_semantic_scholar(self, query: str, max_results: int = 10) -> tuple[list[dict], bool]:
        """
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                logging.warning(f"Semantic Scholar timeout for query '{query}'. Retry {attempt + 1}/{self.MAX_RETRIES}")
                time.sleep(2)
            except requests.exceptions.RequestException as e:
                if attempt < self.MAX_RETRIES - 1:
                    wait_time = (attempt + 1) * 2
                    logging.warning(f"Semantic Scholar request error: {e}. Retry {attempt + 1}/{self.MAX_RETRIES} after {wait_time}s")
                    time.sleep(wait_time)
                else:
                    logging.error(f"Error fetching from Semantic Scholar after {self.MAX_RETRIES} attempts: {e}")
