...
class GeminiClient:
    ...
    def chat(self, prompt: str) -> str:
        ...
        for i in range(retries):
            try:
                resp = requests.post(
                    self.base_url, headers=headers, params=params, json=data, timeout=60
                )
                ...
                if (
                    ...
                    logging.warning(
                        f"API Error ({e.response.status_code}): {e}. Retrying in {wait_time} seconds..."
                    )
                    time.sleep(wait_time)
                else:
                    logging.error(
                        f"Failed to get response from Gemini API after multiple retries: {e}"
                    )
                    raise e
