...
const LoginPage: React.FC = () => {
  ...
  const { login, loginWithOAuth, isAuthenticated, isInitialized } = useAuthStore();
  ...
  const features = [
    { icon: <PsychologyIcon sx={{ color: "#FFB900" }} />, title: "Deep Context", desc: "Agents that remember your goals." },
    { icon: <AutoFixHighIcon sx={{ color: "#00F5C8" }} />, title: "Smart Synthesis", desc: "No more copy-pasting findings." },
    { icon: <HistoryEduIcon sx={{ color: "#00B894" }} />, title: "Audit Trail", desc: "Track every source cited." },
    { icon: <LoginIcon sx={{ color: "#FFB900" }} />, title: "Instant Access", desc: "Pick up right where you left off." },
  ];
      ...
      <FormSection>
        <Box sx={{ width: "100%", maxWidth: "400px" }}>
          ...
          <Typography variant="body1" sx={{ color: "#A1A1AA", mb: 4, fontFamily: "'Crimson Pro', serif", fontSize: "1.1rem" }}>
            Return to your research landscape.
          </Typography>
          ...
          <Box component="form" onSubmit={handleSubmit} noValidate>
            ...
            >
              {loading ? <CircularProgress size={24} sx={{ color: "#09090B" }} /> : "Sign In"}
            </Button>

            <Box sx={{ textAlign: "center", mb: 3 }}>
              <Typography variant="body2" sx={{ color: "#71717A" }}>
                New to Scholar Agent?{" "}
                <Link to="/register" style={{ textDecoration: "none" }}>
                  <span style={{ color: "#00F5C8", fontWeight: 600 }}>Create Account</span>
                </Link>
              </Typography>
            </Box>
          ...
          <Box sx={{ position: "relative", zIndex: 1, maxWidth: "600px", width: "100%" }}>
            <Box sx={{ mb: 6 }}>
              <Typography variant="h2" sx={{ fontWeight: 800, fontSize: "3rem", lineHeight: 1.1, mb: 2, color: "#F4F4F5" }}>
                Focus on the <br />
                <Box component="span" sx={{ color: "#FFB900" }}>Big Picture.</Box>
              </Typography>
              <Typography variant="h6" sx={{ fontFamily: "'Crimson Pro', serif", color: "#A1A1AA", fontWeight: 400, fontSize: "1.35rem", lineHeight: 1.6 }}>
                Log in to resume your deep-dives. Our agents have been keeping your research context warm.
              </Typography>
            </Box>
            ...
            <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 3 }}>
              {features.map((feature, index) => (
                <GlassCard key={index} elevation={0}>
                  ...
                  <Typography variant="h6" sx={{ fontWeight: 700, fontSize: "1rem", mb: 0.5 }}>{feature.title}</Typography>
                  <Typography variant="body2" sx={{ color: "#A1A1AA", fontSize: "0.875rem" }}>{feature.desc}</Typography>
                </GlassCard>
              ))}
            </Box>
          </Box>
